                            ...
                            <c:forEach items="${pageInfo.records}" var="c">
                                <tr>
                                    <td class="dashboard_propert_wrapper">
                                        ...
                                        <div class="title">
                                            <h4><a href="/house/detail/${c.houseId}">${c.house.title}</a></h4>
                                    ...
                                    <td>
                                        ...
                                        <br/>
                                        <!--这里不知道为什么在订单状态为-1即待付款时服务器报500错误无法查看合同，记得去改正-->
                                        <!--这里根据控制台报错后续发现是因为在合同页面中的order变量c获取id的值的时候，将id写成了Id导致从页面后台拿不到对应的属性值而报了500错误-->
                                        <c:if test="${c.status == -2 || c.status == -1}">
                                            <%--                                        <a href="javascript:void(0)" onclick="cancelOrder(${c.id})">取消订单</a>--%>
                                            <!--两个浏览器一直报错没有定义该取消订单的js函数，经过排查一直没有报错，重启后好了-->
                                            <a  href="javascript:void(0)" onclick="cancelOrder(${c.id})">取消订单</a><br/>
                                        </c:if>
                                        <c:if test="${c.status == 0 || c.status == 3}">
                                            <a  href="javascript:void(0)" onclick="endOrder(${c.id})">申请退租</a><br/>
                                        </c:if>
                                        <c:if test="${c.status == 2 && (sessionScope.user.role == 'admin'|| sessionScope.user.role == 'owner')}">
                                            ...
                                            <a  href="javascript:void(0)" onclick="endOrderNotPass(${c.id})">退租申请不通过</a>
                                        </c:if>
                                    </td>
                                </tr>
                            </c:forEach>
